...
def ask_style() -> str:
    """Asks which style=... class option (see olympiad-respa.sty /
    olympiad-ipho.sty) the generated root documents should declare.
    Mainly changes how \\subproblem/\\subsolution and the `task`
    environment are typeset (respa: centered headings, numbered-list
    task env; ipho: colored headings, framed-box task env) -- it does
    NOT change the folder shape this script generates (a солянка
    problem still gets one lettered subfolder per sub-part either way;
    an ipho-style problem that instead reuses \\subproblem/\\subsolution
    several times INSIDE one file, as some real ipho problems do, isn't
    generated by this script and has to be hand-assembled afterwards,
    same as extract-marking.py's own style=respa/ipho choice affects how
    it reads that file back, not how this script writes it)."""
    return choose(
        "Which layout style should the generated documents use "
        "(the style=... class option)?",
        [
            ("respa", "respa"),
            ("ipho", "ipho"),
        ],
...
# ============================================================
def _preamble(*, lang: str, calendar_year: str, grade: str, tour: str, doc_type: str, comp: str, subcomp: str, year_name: str, style: str) -> list[str]:
    lines = [
        r"\documentclass[",
        f"  language={lang},",
...
def render_problems_doc(
    *, groups: list[list[str]], soljanka_indices: set[int], instructions: bool,
    lang: str, calendar_year: str, grade: str, tour: str, comp: str, subcomp: str, year_name: str, style: str,
) -> str:
    lines = _preamble(lang=lang, calendar_year=calendar_year, grade=grade, tour=tour,
                       doc_type="problems", comp=comp, subcomp=subcomp, year_name=year_name, style=style)
    lines += [
        "",
...
def render_solutions_doc(
    *, groups: list[list[str]], soljanka_indices: set[int],
    ...
    lines = _preamble(lang=lang, calendar_year=calendar_year, grade=grade, tour=tour,
                       doc_type="solutions", comp=comp, subcomp=subcomp, year_name=year_name, style=style)
...
def render_jury_doc(
    *, groups: list[list[str]],
    ...
    lines = _preamble(lang=lang, calendar_year=calendar_year, grade=grade, tour=tour,
                       doc_type="jury", comp=comp, subcomp=subcomp, year_name=year_name, style=style)
    lines.append("")
    for i, folders in enumerate(groups):
...
def render_student_marking_doc(
    *, groups: list[list[str]], soljanka_indices: set[int], subpart_points: str, plain_points: str,
    ...
    lines = _preamble(lang=lang, calendar_year=calendar_year, grade=grade, tour=tour,
                       doc_type="marking", comp=comp, subcomp=subcomp, year_name=year_name, style=style)
...
def generate_shape_scaffold(
    ...
    *, grades: list, languages: list[str], instructions: bool, wants_answers: bool,
    wants_student_marking: bool, style: str,
) -> None:
    ...
    print(f"  theory={shape.theory}   experiment={shape.experiment}   style={style}")
    ...
    for grade in grades:
        ...
        for lang in languages:
            ...
            common = dict(lang=lang, calendar_year=calendar_year, grade=str(grade),
                          comp=comp, subcomp=subcomp, year_name=year_name, style=style)
...
def main() -> None:
    ...
    if family == "respa":
        ...
        generate_shape_scaffold(
            ...
            wants_student_marking=wants_student_marking, style="respa",
        )
        return
    ...
    generate_shape_scaffold(
        ...
        wants_student_marking=wants_student_marking, style=style,
    )
